...
@section('title', 'Editando: ' . $userProfiles->description)
...
<div id="event-create-container" class="col-md-6 offset-md-3">
  <div class="shadow-lg p-3 mb-5 bg-white rounded">
    ...
    @endif
      ...
      @csrf
      @method('PUT')
      <div class="shadow-none p-3 mb-5 bg-light rounded"><h1>Editando: {{ $userProfiles->description }}</h1></div>  
      <form  name="formEdit" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="form-group">
        <label for="title">Perfil:</label><tag class="font-red">*</tag>
        <input type="text" class="form-control" id="description" name="description" placeholder="Perfil" value="{{ $userProfiles->description }}" required>
      </div>
      <div class="form-group">
        <label for="title">Ativo?</label><tag class="font-red">*</tag>
        <select name="active" id="active" class="form-control" required>
          <option value="0">Não</option>
          <option value="1"{{ $userProfiles->active == 1 ? "selected='selected'" : "" }}>Sim</option>
        </select>
      </div>
      <BR>
...
<script>
  $(function(){
      ...
      $.ajax({
        url: "/userProfile/updateUserProfile/{{$userProfiles->id}}",
        type: "POST",
        data: $(this).serialize(),
        dataType: 'json',
