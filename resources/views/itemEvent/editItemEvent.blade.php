...
@section('title', 'Editando: ' . $item->description)
...
<div id="event-create-container" class="col-md-6 offset-md-3">
  <div class="shadow-lg p-3 mb-5 bg-white rounded">
    ...
    @endif
      ...
      @csrf
      @method('PUT')
      <div class="shadow-none p-3 mb-5 bg-light rounded"><h1>Editando: {{ $item->description }}</h1></div>  
      <form  name="formEdit" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="form-group">
        <label for="title">Item:</label><tag class="font-red">*</tag>
        <input type="text" class="form-control" id="description" name="description" placeholder="Perfil" value="{{ $item->description }}" required>
      </div>
      <div class="form-group">
        <label for="title">Ativo?</label><tag class="font-red">*</tag>
        <select name="active" id="active" class="form-control" required>
          <option value="0">Não</option>
          <option value="1"{{ $item->active == 1 ? "selected='selected'" : "" }}>Sim</option>
        </select>
      </div>
      <BR>
      <input type="submit" id="buttonSubmit"  class="btn btn-primary" value="Atualizar">
      <a href="/itemEvent/show" id="btn_cancelar" class="btn btn-outline-warning">Voltar</a>
    </form>
  </div>
</div>
<script>
  $(function(){
      ...
      $.ajax({
        url: "/itemEvent/updateItemEvent/{{$item->id}}",
        type: "POST",
        data: $(this).serialize(),
        dataType: 'json',
